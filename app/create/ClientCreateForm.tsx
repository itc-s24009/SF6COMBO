...
import { createCombo } from "@/app/action";
...
export default function ClientCreateForm({ allTags }: { allTags: { id: string; name: string }[] }) {
  ...
  const removeMove = (indexToRemove: number) => setSelectedMoves(selectedMoves.filter((_, index) => index !== indexToRemove));
        ...
        <div className="p-3 max-w-4xl mx-auto flex flex-col gap-2">
          ...
          <div className="min-h-[50px] flex flex-wrap gap-2 items-center relative">
            ...
            {selectedMoves.map((move, index) => (
              <button type="button" key={index} onClick={() => removeMove(index)} className="group relative bg-zinc-800 border border-zinc-700 text-zinc-100 px-2 py-1 rounded text-xs font-bold hover:bg-red-900/50 hover:border-red-500 hover:text-red-200 transition-all">
                {move}
        ...
        <form action={createCombo} className="space-y-6 pt-4 border-t border-zinc-800">
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2 mb-2 min-h-[24px]">
              {tags.map((tag) => (
                <span key={tag} onClick={() => setTags(tags.filter(t => t !== tag))} className="bg-violet-900/30 border border-violet-800 text-violet-300 px-2 py-0.5 rounded text-[10px] font-bold cursor-pointer hover:bg-red-900/30 hover:text-red-300 transition flex gap-1 group">
            ...
            {availableTags.length > 0 && (
              <div className="flex flex-wrap gap-1 mt-1 p-2 bg-zinc-950 rounded border border-zinc-800">
                <span className="w-full text-[10px] text-zinc-600 font-bold mb-1 uppercase">Select Existing Tags</span>
                {availableTags.map(tag => (
                  <button type="button" key={tag.id} onClick={() => addExistingTag(tag.name)} className="bg-zinc-900 border border-zinc-700 text-zinc-400 hover:text-zinc-100 hover:bg-zinc-800 hover:border-zinc-500 px-2 py-1 rounded text-[10px] font-bold transition">
                    + {tag.name}
                  </button>
                ))}
              </div>
